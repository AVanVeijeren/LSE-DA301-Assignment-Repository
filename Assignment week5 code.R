...
sales_byProduct_global <- arrange(sales_byProduct, global_sales)
bottom_ten_global <- sales_byProduct[1:11,]
bottom_ten_global

# In the EU? 
sales_byProduct_EU <- arrange(sales_byProduct, eu_sales)
bottom_ten_eu <- sales_byProduct[1:11,]
bottom_ten_eu

# In the NA? 
sales_byProduct_NA <- arrange(sales_byProduct, na_sales)
...
View(sales_df)
head(sales_df)
sales_df <- subset(sales_df, select = -c(X, Product))
...
# Reminder: outliers were removed at the beginning of the R assignment. 
...
# Test if log transformation works better
sales_df_transform <- mutate(sales_df2, global_sales_log = log(global_sales))
shapiro.test(sales_df_transform$global_sales_log)
...
## EU and NA sales are correlated.
...
# The qq-plot suggests that the data is normally distributed but the 
# Shapiro-Wilk test suggests otherwise. 
